...
class ConvModule(nn.Module):
    def __init__(self,in_channel,out_channel,channel1,channel2):
        super(ConvModule, self).__init__()
        # 定义六层卷积层
        # 两层HDC（1,2,5,1,2,5）
        self.conv1 = nn.Sequential(
            # 第一层 (3-1)*1+1=3 （64-3)/1 + 1 =62
            nn.Conv2d(in_channels=channel1+channel2, out_channels=out_channel, kernel_size=1, stride=1, padding=0),
        ...
        self.conv2 = nn.Sequential(
            # 第一层 (3-1)*1+1=3 （64-3)/1 + 1 =62
            nn.Conv2d(in_channels=channel1+channel2, out_channels=out_channel, kernel_size=3, stride=1, padding=0, dilation=6),
            nn.BatchNorm2d(out_channel),
            # inplace-选择是否进行覆盖运算
            nn.ReLU(inplace=True)
        )
        self.conv3 = nn.Sequential(
            # 第一层 (3-1)*1+1=3 （64-3)/1 + 1 =62
            nn.Conv2d(in_channels=channel1+channel2, out_channels=out_channel, kernel_size=3, stride=1, padding=0, dilation=12),
            nn.BatchNorm2d(out_channel),
            # inplace-选择是否进行覆盖运算
            nn.ReLU(inplace=True)
        )
        self.conv4 = nn.Sequential(
            # 第一层 (3-1)*1+1=3 （64-3)/1 + 1 =62
            nn.Conv2d(in_channels=channel1+channel2, out_channels=out_channel, kernel_size=3, stride=1, padding=0, dilation=18),
            nn.BatchNorm2d(out_channel),
            # inplace-选择是否进行覆盖运算
            nn.ReLU(inplace=True)
        )
        ...
        self.unsample = nn.ConvTranspose2d(channel2, channel1, kernel_size=2, stride=2)
    ...
    def forward(self,x,y):
        ...
        for i in range(0,4):
            if p2[i].shape != p1[i].shape: #用于判断你粘贴前，这两个对象是不是一样大。
                p2[i] = TF.resize(p2[i], size=p1[i].shape[2:])
            p1[i]=torch.cat([p1[i],p2[i]],dim=1)

        # p1.append(p2[3])
        # p1[4]=self.conv0(p1[4])
        p1[3]=self.conv1(p1[3])
        p1[2]=self.conv2(p1[2])
        p1[1]=self.conv3(p1[1])
        p1[0]=self.conv4(p1[0])
        ...
        _,_,w,h=x1.shape
        p1[0] = TF.resize(p1[0],size=[w,h])
        p1[1] = TF.resize(p1[1],size=[w,h])
        p1[2] = TF.resize(p1[2],size=[w,h])
        p1[3] = TF.resize(p1[3],size=[w,h])
        # p1[4] = TF.resize(p1[4],size=[w,h])

        output=torch.cat([p1[0],p1[1],p1[2],p1[3]],dim=1)
...
class atrous_conv(nn.Module):
    def __init__(self,in_channel,out_channel,channel1,channel2):
        ...
        self.up=nn.Upsample(scale_factor=2,mode='bilinear')
        self.pc=ConvModule(in_channel,out_channel,channel1,channel2)
        self.conv=nn.Conv2d(channel1*5+channel2,channel1,1)
        # self.conv1=nn.Conv2d(out_channel*4,out_channel*4,1)
...
class v1(nn.Module):
    def __init__(self,*,in_channel=3,out_channel=1):
        ...
        self.sub_channel = sub_channel_attention(channels=1024, kernel_size=3, ratio=7)

        self.atrous3=atrous_conv(256,256,256,512)
        self.atrous2=atrous_conv(128,128,128,256)
        self.atrous1=atrous_conv(64,64,64,128)
    ...
    def forward(self,x):
        ...
        skip_connection2[3]=self.sub_channel(skip_connection2[3],x2)
        x=x1+x2
        x=self.upsample(x)
