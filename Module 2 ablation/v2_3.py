...
class ConvModule(nn.Module):
    def __init__(self,in_channel,out_channel):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv2d(in_channels=in_channel, out_channels=out_channel, kernel_size=3, stride=1, padding=0, dilation=1),
        ...
        self.conv2 = nn.Sequential(
            # 第一层 (3-1)*1+1=3 （64-3)/1 + 1 =62
            nn.Conv2d(in_channels=in_channel, out_channels=out_channel, kernel_size=3, stride=1, padding=0, dilation=2),
            nn.BatchNorm2d(out_channel),
            # inplace-选择是否进行覆盖运算
            nn.ReLU(inplace=True)
        )
        self.conv3 = nn.Sequential(
            # 第一层 (3-1)*1+1=3 （64-3)/1 + 1 =62
            nn.Conv2d(in_channels=in_channel, out_channels=out_channel, kernel_size=3, stride=1, padding=0, dilation=3),
            nn.BatchNorm2d(out_channel),
            # inplace-选择是否进行覆盖运算
            nn.ReLU(inplace=True)
        )
        self.conv4 = nn.Sequential(
            # 第一层 (3-1)*1+1=3 （64-3)/1 + 1 =62
            nn.Conv2d(in_channels=in_channel, out_channels=out_channel, kernel_size=3, stride=1, padding=0, dilation=5),
            nn.BatchNorm2d(out_channel),
            # inplace-选择是否进行覆盖运算
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x):
        ...
        _,_,w,h=x.shape
        p1 = TF.resize(p1,size=[w,h])
        p2 = TF.resize(p2,size=[w,h])
        p3 = TF.resize(p3,size=[w,h])
        p4 = TF.resize(p4,size=[w,h])
...
class atrous_conv(nn.Module):
    def __init__(self,in_channel,out_channel):
        ...
        self.up=nn.Upsample(scale_factor=2,mode='bilinear')
        self.pc=ConvModule(in_channel,out_channel)
        self.conv=nn.Conv2d(in_channel+out_channel*4,out_channel,1)
...
class v1(nn.Module):
    def __init__(self,*,in_channel=3,out_channel=1):
        ...
        self.atrous3=atrous_conv(768,256)
        self.atrous2=atrous_conv(384,128)
        self.atrous1=atrous_conv(192,64)
