...
class vit_transformer(nn.Module):
    def __init__(self, *, in_channel,out_channel,image_size, patch_size, dim, heads, mlp_dim, pool='cls', channels=3,
        ...
        self.dw_conv=DepthWiseConv(115,out_channel)
        self.dw_conv1=DepthWiseConv(179,out_channel)
        self.dw_conv2=DepthWiseConv(283,out_channel)
        self.dw_conv3=DepthWiseConv(521,out_channel)
        self.conv=nn.Conv2d(out_channel,out_channel,1)
        self.pool=nn.MaxPool2d(2)
    def forward(self,input,level1,level2,level3,n,k):
        l=n
        # input表示原图, level1是第一层的结果，level2是第二层的结果，level3代表要进行local_branch的图像，
        x=level3
        if(k==0):
            for i in range(n):
...
class encoder2(nn.Module):
    def __init__(self,in_channel):
        ...
        self.transformer1 = vit_transformer(in_channel=3,out_channel=64,image_size=112, patch_size=28, dim=196, heads=8, mlp_dim=2048,channels=256)
        self.transformer2 = vit_transformer(in_channel=64,out_channel=128,image_size=112, patch_size=28, dim=196, heads=8, mlp_dim=2048,channels=256)
        self.transformer3 = vit_transformer(in_channel=128,out_channel=256,image_size=112,patch_size=28, dim=196, heads=8, mlp_dim=2048,channels=128)
        self.transformer4 = vit_transformer(in_channel=256,out_channel=512,image_size=56, patch_size=28, dim=196, heads=8, mlp_dim=2048,channels=256)

        self.bottleneck=Bottleneck(512,1024,1024)
    def forward(self,x,y,z):
        skip_connection=[]
        e1=self.transformer1(x,y,z,x,0,1)
        skip_connection.append(e1)
        e1=self.pool(e1)

        e2=self.transformer2(x,skip_connection[0],y,e1,1,0)
        skip_connection.append(e2)
        e2 = self.pool(e2)

        e3=self.transformer3(x,skip_connection[0],skip_connection[1],e2,2,0)
        skip_connection.append(e3)
        e3=self.pool(e3)

        e4=self.transformer4(x,skip_connection[0],skip_connection[2],e3,3,0)
...
class encoder1(nn.Module):
    ...
    def forward(self,x):
        ...
        e4=self.encoder4(e3)
        skip_connection.append(e4)
        x1=self.bottleneck(e4)

        return skip_connection,x1,x
...
class v1(nn.Module):
    ...
    def forward(self,x):
        skip_connection1,x1,x2=self.encoder_first(x)
        skip_connection2,x2=self.encoder_second(x,x2,skip_connection1[0])
