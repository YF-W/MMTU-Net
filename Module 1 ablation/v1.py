...
class vit_transformer(nn.Module):
    def __init__(self, *, in_channel,out_channel,image_size, patch_size, dim, heads, mlp_dim, pool='cls', channels=3,
        ...
        self.dw_conv1=DepthWiseConv(518,out_channel)
        self.conv=nn.Conv2d(out_channel,out_channel,1)
        self.pool=nn.MaxPool2d(2)
    def forward(self, level1,level2,level3,input,n):
        l=n
...
class encoder2(nn.Module):
    def __init__(self,in_channel):
        ...
        self.re1=Res1(in_channel,64)
        self.re2=Res1(64,128)
    ...
    def forward(self,x):
        ...
        e2 = self.pool(e2)
        e3=self.transformer1(skip_connection[0],skip_connection[1],e2,x,2)
        skip_connection.append(e3)
        e3=self.pool(e3)
        e4=self.transformer2(skip_connection[0],skip_connection[2],e3,x,3)
...
#         self.conv1=nn.Conv2d(in_channels=channels,out_channels=channels,kernel_size=1,stride=2,padding=13)
#         self.relu=nn.ReLU()
...
class sub_channel_attention(nn.Module):
    def __init__(self,channels,kernel_size,ratio=7):
        ...
        self.decoder_unsample2=nn.ConvTranspose2d(channels2, channels2, kernel_size=2, stride=2)
    ...
    def forward(self,y,x):
        ...
        x2=self.bn(x2)
        x2=self.sigmoid(x2)
        x=self.decoder_unsample1(x)
        x2=self.decoder_unsample2(x2)
...
class v1(nn.Module):
    def __init__(self,*,in_channel=3,out_channel=1):
        ...
        self.sub_channel1=sub_channel_attention(channels=512,kernel_size=3,ratio=7)
        self.sub_channel2=sub_channel_attention(channels=1024,kernel_size=3,ratio=7)
        self.decoder4=DoubleConv(1536,512)
        self.decoder3=DoubleConv(768,256)
        self.decoder2=DoubleConv(384,128)
        self.decoder1=DoubleConv(192,64)
        self.conv=nn.Conv2d(64,out_channel,1)
        self.unsample1=nn.ConvTranspose2d(1024,512,kernel_size=2,stride=2)
        self.unsample2=nn.ConvTranspose2d(512,512,kernel_size=2,stride=2)
        self.unsample3=nn.ConvTranspose2d(512,256,kernel_size=2,stride=2)
        self.unsample4=nn.ConvTranspose2d(256,256,kernel_size=2,stride=2)
        self.unsample5=nn.ConvTranspose2d(256,128,kernel_size=2,stride=2)
        self.unsample6=nn.ConvTranspose2d(128,128,kernel_size=2,stride=2)
        self.unsample7=nn.ConvTranspose2d(128,64,kernel_size=2,stride=2)
        self.unsample8=nn.ConvTranspose2d(64,64,kernel_size=2,stride=2)
    ...
    def forward(self,x):
        ...
        skip_connection2[3]=self.sub_channel2(skip_connection2[3],x2)
        x=x1+x2
        x=self.unsample1(x)
        skip_connection1[3]=self.unsample2(skip_connection1[3])
        d4=torch.cat([skip_connection1[3],skip_connection2[3],x],dim=1)
        d4=self.decoder4(d4)
        skip_connection2[2]=self.sub_channel1(skip_connection2[2],d4)
        d4=self.unsample3(d4)
        skip_connection1[2]=self.unsample4(skip_connection1[2])
        d3=torch.cat([skip_connection2[2],skip_connection1[2],d4],dim=1)
        d3=self.decoder3(d3)
        d3 = self.unsample5(d3)
        skip_connection1[1] = self.unsample6(skip_connection1[1])
        d2=torch.cat([skip_connection1[1],skip_connection2[1],d3],dim=1)
        d2=self.decoder2(d2)
        d2 = self.unsample7(d2)
        skip_connection1[0] = self.unsample8(skip_connection1[0])
