...
class ConvModule(nn.Module):
    def __init__(self,in_channel,out_channel,channel1,channel2):
        ...
        self.conv2 = nn.Sequential(
            # 第一层 (3-1)*1+1=3 （64-3)/1 + 1 =62
            nn.Conv2d(in_channels=in_channel, out_channels=out_channel, kernel_size=3, stride=1, padding=0, dilation=6),
            nn.BatchNorm2d(out_channel),
            # inplace-选择是否进行覆盖运算
            nn.ReLU(inplace=True)
        )
        self.conv3 = nn.Sequential(
            # 第一层 (3-1)*1+1=3 （64-3)/1 + 1 =62
            nn.Conv2d(in_channels=in_channel, out_channels=out_channel, kernel_size=3, stride=1, padding=0, dilation=12),
            nn.BatchNorm2d(out_channel),
            # inplace-选择是否进行覆盖运算
            nn.ReLU(inplace=True)
        )
        self.conv4 = nn.Sequential(
            # 第一层 (3-1)*1+1=3 （64-3)/1 + 1 =62
            nn.Conv2d(in_channels=in_channel, out_channels=out_channel, kernel_size=3, stride=1, padding=0, dilation=18),
            nn.BatchNorm2d(out_channel),
            # inplace-选择是否进行覆盖运算
            nn.ReLU(inplace=True)
        )
